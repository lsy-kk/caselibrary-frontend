<template>
  ...
  <el-collapse v-model="activeNames">
    <el-collapse-item title="标题" name="1" class="mb-4">
      <!--案例头部信息-->
      <el-form 
          ref="caseHeaderRef"
          v-model = "caseHeader" 
    ...
    <el-collapse-item title="附件" name="2" class="mb-4">
      <!--展示已上传的附件-->
      <!--上传附件，手动上传-->
      <el-upload
  ...
  <div class="fixed inset-x-0 bottom-0 text-center">
    ...
    <el-button @click="handleSave" class="w-2/5">保存</el-button>
  </div>
</template>
...
import type { 
  ...
  UploadProps, 
  UploadUserFile, 
  UploadRawFile } from 'element-plus'
...
const reload = () => {
  ...
  if (props.caseId !== undefined){
    getCaseHeader(props.caseId).then((res)=>{
      if (res.msg === 'success'){
        caseHeader.value = res.data 
      }
    })
    getCaseBodyByCaseId(props.caseId).then((res) => {
      if (res.msg === 'success'){
...
const handleSave = () => {
  if (caseHeader.value.id === undefined){
    ...
    // 同时更新header和body
    Promise.all([pCaseHeaderSave, pCaseBodySave]).then(() => {
      ElMessage.success("保存成功")
    })
    .catch((error) => {
      ElMessage.error("保存失败")
